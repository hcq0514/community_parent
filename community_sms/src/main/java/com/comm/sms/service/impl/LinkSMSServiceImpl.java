...
//import com.cbwleft.sms.constant.SmsChannels;
...
//import com.cbwleft.sms.dao.model.Sms;
...
//import com.cbwleft.sms.model.dto.SendSmsResult;
//import com.comm.sms.service.IBatchQueryable;
//import com.comm.sms.service.IMessageService;
...
//
//	@Override
//	public SendSmsResult send(App app, Template template, MessageDTO message) throws ChannelException {
...
//		if (intBody > 0) {
//			return new SendSmsResult(true, body);
//		} else {
//			logger.warn("凌凯短信发送失败,错误代码:{}", body);
//			return new SendSmsResult(body);
//		}
//	}
//
...
//
//	@Override
//	public QuerySendResult querySendStatus(App app, Sms message) {
...
//					String response = args[4];// 报告：各运营商直接返回的状态报告值
//					Date receiveDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(args[5]);// 2018-05-29 10:36:48
//					Sms message = messageService.queryMessage(mobile, bizId);
//					if (message != null) {
...
//					short status = stringRedisTemplate.opsForSet().size(RedisKeys.BATCH_MESSAGE_SUCCESS.format(id)).shortValue();
//					short failure = stringRedisTemplate.opsForSet().size(RedisKeys.BATCH_MESSAGE_FAILURE.format(id)).shortValue();
//					messageService.updateBatchMessageCount(id, sending, status, failure);
//				} catch (Exception e) {
...
//
//	@Override
//	public SendSmsResult batchSend(App app, String[] mobile, String content) throws ChannelException {
//		try {
//			content += "【" + app.getPrefix() + "】";
//			MultiValueMap<String, Object> params = new LinkedMultiValueMap<>();
//			params.add("CorpID", linkSMSConfig.getCorpId());
